...
DESIRED_FPS = 30             # UI/display FPS (increased for smoother display)
INFER_FPS = 10               # Increased from 8 to 10 for better responsiveness
TFLITE_THREADS = max(1, multiprocessing.cpu_count() - 1)
NAIL_COLOR = (199, 21, 133)
TEXTURE_DEFAULT = 0.35
MIN_CONTOUR = 60
DILATION_PIXELS = 3          # Reduced from 4
FEATHER_IN = 2               # Reduced from 3
FEATHER_OUT = 4              # Reduced from 6
MAX_OUT_ALPHA = 0.08
MASK_DOWNSCALE = 0.4         # Reduced from 0.5 for faster processing
...
def create_feathered_alpha(mask, inner_feather=2, outer_feather=4, max_out_alpha=0.10):
    """Optimized alpha channel creation"""
    m = (mask > 127).astype(np.uint8) * 255
    if m.sum() == 0:
        return np.zeros_like(m, dtype=np.float32)

    # Use faster distance transform
    dist_in = cv2.distanceTransform(m, cv2.DIST_L2, 3).astype(np.float32)
    dist_out = cv2.distanceTransform(255 - m, cv2.DIST_L2, 3).astype(np.float32)
...
def colorize_nail_texture(orig_rgb, alpha, color_rgb, texture_strength=0.35, brightness_adjust=1.0):
    ...
    gray = cv2.cvtColor(orig_rgb, cv2.COLOR_RGB2GRAY).astype(np.float32) * (1.0/255.0)
    ...
    # Simplified highlight boost
    highlight_boost = highlights[:, :, None] * 0.2
    colored_layer *= (1 + highlight_boost)
    ...
    def run(self):
        while self.running:
            ...
            if not ret:
                time.sleep(0.005)
                continue
            # Always get fresh frame, discard old
            if not self.q.empty():
                try:
                    _ = self.q.get_nowait()
                except queue.Empty:
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                resized = cv2.resize(frame_rgb, (self.in_w, self.in_h), interpolation=cv2.INTER_AREA)
                ...
                if not combined_mask.any():
                    ...
                    alpha_map = cv2.resize(alpha_small, (W, H), interpolation=cv2.INTER_AREA)
    ...
    def _build_fallback_mask(self, outputs, H, W):
        ...
        for idx, o in enumerate(outputs):
            ...
            for m in candidate:
                ...
                mask_resized = cv2.resize(mask_uint8, (W, H), interpolation=cv2.INTER_AREA)
